...
export const ImportedMoviesList = () => {
  ...
  const handleDelete = async (tmdbId: number) => {
    ...
    try {
      const { error } = await supabase.from("movies_imported").delete().eq("tmdb_id", tmdbId)
  ...
  if (movies.length === 0) {
    return <div className="text-center py-8 text-muted-foreground">No hay películas importadas</div>
  }
          ...
          <div key={movie.id} className="group relative">
            <div className="relative aspect-[2/3] rounded-lg overflow-hidden">
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <Button size="icon" variant="secondary" onClick={() => handleOpenEdit(movie)}>
                  <Pencil className="w-4 h-4" />
                </Button>
                <Button size="icon" variant="destructive" onClick={() => handleDelete(movie.id)}>
      ...
      <Dialog open={!!editingMovie} onOpenChange={(open) => !open && setEditingMovie(null)}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <Button onClick={handleEdit} disabled={!videoUrl.trim()} className="w-full">
              Actualizar Película
            </Button>
          </div>
        </DialogContent>
