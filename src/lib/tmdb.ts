...
export const getMovieGenres = async (language = "es-MX"): Promise<Genre[]> => {
  if (movieGenresCache[language]) {
    return movieGenresCache[language]
  }

  try {
    const response = await fetch(`${TMDB_BASE_URL}/genre/movie/list?api_key=${TMDB_API_KEY}&language=${language}`)
    if (!response.ok) {
...
export const getTVShowGenres = async (language = "es-MX"): Promise<Genre[]> => {
  if (tvShowGenresCache[language]) {
    return tvShowGenresCache[language]
  }

  try {
    const response = await fetch(`${TMDB_BASE_URL}/genre/tv/list?api_key=${TMDB_API_KEY}&language=${language}`)
    if (!response.ok) {
...
export const getOptimizedImageUrl = (path: string | null | undefined, containerWidth = 500): string => {
  if (!path) return ""

  // Select optimal size based on container
  let size = "w500"
  if (containerWidth <= 92) size = "w92"
  else if (containerWidth <= 154) size = "w154"
  else if (containerWidth <= 185) size = "w185"
  else if (containerWidth <= 342) size = "w342"
  else if (containerWidth <= 500) size = "w500"
  else size = "w780"
...
export const getTrendingTVShows = async () => {
  const response = await fetch(`${TMDB_BASE_URL}/trending/tv/week?api_key=${TMDB_API_KEY}&language=es-MX`)
  const data = await response.json()
...
export const getLogo = async (type: "movie" | "tv", id: number) => {
  const response = await fetch(`${TMDB_BASE_URL}/${type}/${id}/images?api_key=${TMDB_API_KEY}`)
  const data = await response.json()
  ...
  const findBestLogo = (logos: Logo[], lang: string | null) => {
    const langLogos = lang ? logos.filter((logo) => logo.iso_639_1 === lang) : logos.filter((logo) => !logo.iso_639_1)
...
export const getPopularMovies = async () => {
  const response = await fetch(`${TMDB_BASE_URL}/movie/popular?api_key=${TMDB_API_KEY}&language=es-MX`)
  const data = await response.json()
  return data.results as Movie[]
}

export const getPopularTVShows = async () => {
  const response = await fetch(`${TMDB_BASE_URL}/tv/popular?api_key=${TMDB_API_KEY}&language=es-MX`)
  const data = await response.json()
  return data.results as TVShow[]
}
...
export const searchTVShows = async (query: string) => {
  const response = await fetch(
    `${TMDB_BASE_URL}/search/tv?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}&language=es-MX`,
  )
  const data = await response.json()
  return data.results as TVShow[]
}
